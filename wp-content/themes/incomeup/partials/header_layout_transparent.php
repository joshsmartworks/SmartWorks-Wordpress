<?php
	...
	$switch_header_class = (get_theme_mod( 'header_with_switch', false)) ? ' switch_main_header' : '';

?>

<header id="ABdev_main_header" class="clearfix transparent <?php echo esc_attr($sticky_header_class) . esc_attr($switch_header_class); ?>">
		...
		<div class="container">
			<div class="row">
				<div id="header_social_info" class="span8">
					...
					if($header_email!=''): ?>
						<span class="quick_contact_mail"><i class="ci_icon-envelope"></i><a href="<?php echo esc_url('mailto:'.$header_email)?>"><?php echo esc_html($header_email); ?></a></a></span>
					<?php endif; ?>
				</div>
				...
				<?php if( in_array('woocommerce/woocommerce.php', get_option('active_plugins')) ):?>
					...
					<?php endif; ?>
						| <i class="ci_icon-shopping-cart"></i><?php _e(' Cart:','ABdev_incomeup') ?>
						<a class="link_cart" href="<?php echo esc_url($woocommerce->cart->get_cart_url()); ?>">
							<span>
							<?php if($woocommerce->cart->cart_contents_count==1){
								echo $woocommerce->cart->cart_contents_count . __(' item','ABdev_incomeup');
								} else{
								echo $woocommerce->cart->cart_contents_count . __(' items','ABdev_incomeup');
								}
							?>
							</span>
						</a>
		...
		<div class="container">
			<div id="logo">
				<a href="<?php echo home_url(); ?>">
					...
					$header_inverted_logo = get_theme_mod('inverted_header_logo', '');
					$header_inverted_retina_logo = get_theme_mod('header_inverted_retina_logo', '');
					$header_inverted_retina_logo_width = get_theme_mod('header_inverted_retina_logo_width', '');
					$header_inverted_retina_logo_height = get_theme_mod('header_inverted_retina_logo_height', '');
					if( $header_logo!='' ):?>
					...
					<?php if( $header_retina_logo!='' &&  $header_retina_logo_width!='' && $header_retina_logo_height!='' ):?>
						...
						<?php if($header_inverted_logo != ''): ?>
							<img id="inverted_logo" src="<?php echo $header_inverted_logo; ?>" alt="<?php bloginfo('name');?>">
							<?php if( $header_inverted_retina_logo!='' &&  $header_inverted_retina_logo_width!='' && $header_inverted_retina_logo_height!='' ):?>
								<?php $pixels ="";
								if(is_numeric($header_inverted_retina_logo_width) && is_numeric($header_inverted_retina_logo_height)):
								$pixels ="px";
								endif; ?>
								<img id="inverted_retina_logo" src="<?php echo $header_inverted_retina_logo;?>" alt="<?php bloginfo('name');?>" style="width:<?php echo $header_inverted_retina_logo_width.$pixels; ?>;max-height:<?php echo $header_inverted_retina_logo_height.$pixels; ?>; height: auto !important;">
							<?php endif; ?>
						<?php endif; ?>
					<?php endif; ?>
					<?php else: ?>
...
<div id="ABdev_header_spacer" class="transparent"></div>
