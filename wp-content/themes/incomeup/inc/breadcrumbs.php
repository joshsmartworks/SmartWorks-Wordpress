...
if ( ! function_exists( 'ABdevFW_simple_breadcrumb' ) ){
	function ABdevFW_simple_breadcrumb(){
		$text['home']     = __('Home', 'ABdev_incomeup');
		$text['portfolio']= __('Portfolio', 'ABdev_incomeup');
		$text['forum']= __('Forum', 'ABdev_incomeup');
		$text['tax'] 	  = __('Archive for "%s"', 'ABdev_incomeup');
		$text['search']   = __('Search Results for "%s"', 'ABdev_incomeup');
		$text['tag']      = __('Posts Tagged "%s"', 'ABdev_incomeup');
		$text['author']   = __('Articles by %s', 'ABdev_incomeup');
		$text['404']      = __('Error 404', 'ABdev_incomeup');
		$delimiter   = ' <i class="ci_icon-chevron-right"></i> '; // delimiter between crumbs
		...
		$link = '<a href="%s">%s</a>';
			...
			echo '<div class="breadcrumbs">' . sprintf($link, esc_url($homeLink), $text['home']) . $delimiter;
			...
			if ( get_query_var('paged') ) {
				...
				echo __('Page', 'ABdev_incomeup') . ' ' . get_query_var('paged');
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
