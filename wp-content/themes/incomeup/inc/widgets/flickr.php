<?php
class ABdev_incomeup_flickr_stream extends WP_Widget {

	function ABdev_incomeup_flickr_stream(){
		$widget_ops = array(
			'classname' => 'flickr-stream',
			'description' => __('Photostream from Flickr.', 'ABdev_incomeup'),
		);
		$control_ops = array(
			'id_base' => 'flickr-stream',
		);
		parent::__construct('flickr-stream', __('Flickr stream', 'ABdev_incomeup' ), $widget_ops, $control_ops);
	}
			...
			if(is_object($stream) && isset($stream->user->id)) {
				...
				<div class='flickr_stream clearfix'>
					<?php foreach($photos->photos->photo as $photo): $photo = (array) $photo; ?>
							<a class="link-middle-image" href="<?php echo esc_url($photos_url->user->url); ?><?php echo $photo['id']; ?>" target='_blank'>
							<img src="<?php $url = "https://farm" . $photo['farm'] . ".static.flickr.com/" . $photo['server'] . "/" . $photo['id'] . "_" . $photo['secret'] . '_s' . ".jpg"; echo esc_url($url); ?>" alt="">
							</a>
					<?php endforeach; ?>
				</div>
				<?php
			} else {
				echo '<p>'.__('Invalid flickr username or connection error.', 'ABdev_incomeup').'</p>';
			}
		}
		echo $after_widget;
	}
	...
	function form($instance){
		$defaults = array('title' => __('Photos from Flickr', 'ABdev_incomeup'), 'flickr_username' => '', 'number' => 9);
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('flickr_username')); ?>"><?php _e('Flickr username:', 'ABdev_incomeup');?></label>
			<input type="text" class="widefat" id="<?php echo esc_attr($this->get_field_id('flickr_username')); ?>" name="<?php echo esc_attr($this->get_field_name('flickr_username')); ?>" value="<?php echo esc_attr($instance['flickr_username']); ?>" />
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php _e('No. of images:', 'ABdev_incomeup');?></label>
...
function ABdev_incomeup_flickr_stream_widget(){
	register_widget('ABdev_incomeup_flickr_stream');
}

add_action('widgets_init', 'ABdev_incomeup_flickr_stream_widget');