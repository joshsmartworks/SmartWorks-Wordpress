	...
	<div class="container_fullwidth">
		...
		if ($posts->have_posts()){
			while ($posts->have_posts()){
				...
				$out.= '<div class="portfolio_item portfolio_masonry_fullwidth '.esc_attr( $portfolio_data["ABp_portfolio_masonry_image_size"][0] ).' '.$slugs.'">
					<div class="overlayed">
		...
		if(isset($filter_slugs) && is_array($filter_slugs)){
			foreach($filter_slugs as $slug => $name){
				$filter_out.='<li><a href="#filter_'.esc_attr($unique_filter_id).'" class="portfolio_filter_button" data-option-value=".'.$slug.'">'.$name.'</a></li>';
			}
		}
		?>
